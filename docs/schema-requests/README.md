...
| Who writes it | This repo *authors* it as a request file here — the exact DDL, the feature it unblocks, and what changes in web |
...
- **Wrap in `begin;` / `commit;`.** A paste that fails halfway must not leave a table created *without* its unique index. This is the reason web's own migration is wrapped, and it applies to every request here.
