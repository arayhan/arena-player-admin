...
**No new index — the arithmetic behind it changed on 2026-08-15.** This was arithmetic, not taste: 9 slots × 14 days capped active rows at 126. It is now 18 slots × 92 days = **1,656**, and lifetime accumulation grows at twice the old rate. The conclusion has **not** been re-derived at the new ceiling and is flagged as an open question in [architecture.md](architecture.md)'s bookings-list contract, which owns the decision. Do not add an index from this line, and do not read it as settled either.
