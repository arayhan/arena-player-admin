...
| **Queue not connected**     | `/` until Phase 2 lands         | **"Cannot see", never "nothing to do".** No `bookings` table exists, so the app has not looked and found the queue empty — it cannot look at all. Shipped 2026-08-15 in `(dashboard)/page.tsx` with that distinction in a comment, because it is one edit away from becoming the reassuring lie    |

> **DECIDED 2026-08-15 — the empty queue does NOT reuse `EmptyState`.**
>
> `EmptyState` is centered, `py-12`, an `<h2>` plus a muted paragraph, no green and no completion mark. That is the universal **"no results"** shape, and the row above requires the empty queue to read as **earned success**. One component cannot carry both meanings, and a design review found that the visual grammar — not the copy — is what fails: changing the words inside a muted centered box still reads as a failed lookup.
>
> So Phase 6 builds a **separate done-state component** using the **green triple** (`green-bg` / `green-border` / `green-ink`, the same surface + border + ink rule every status pill follows), and `EmptyState` keeps the neutral no-results job it already has — which is the correct component for the **"No results for a filter"** row above.
>
> Recorded here rather than left to the build because [2-step-02](2-step-02-list.md) would otherwise wire `EmptyState` into the queue by default, and nothing would fail: the queue would simply spend most of its life telling the admin their finished day looks like a failed search.

## Ranges — design against the real numbers

- **1,656 active bookings maximum**: 18 slots × 92 days, since the 2026-08-15 domain re-copy. This line previously read 126 (9 slots × 14 days) and concluded "pagination exists for comfort, not for scale." **That conclusion is now wrong.** At 50/page a fully-booked window is **34 pages**, and even a normal week of pending rows can exceed one page — pagination is load-bearing, and the page control is a primary navigation element rather than a courtesy at the bottom of a list that always fit anyway.
- **The 375px card list is the surface this hits hardest**, because it was specified against the old ceiling. [DESIGN.md](../DESIGN.md) puts one card per booking below 720px; a 50-row page that was ~2 screens of desktop table is a long scroll of cards on a phone, and "scroll to the pager" stops being free. The consequence is a design question, not a number to update: whether the phone breakpoint needs a smaller page size, a sticky pager, or a denser card is unresolved here and belongs to whoever builds the responsive list. What is settled is that it can no longer be waved through on the grounds that the whole set fits.
- Virtualisation and infinite scroll are still **not** justified — 1,656 is small, and neither is on the table. The index decision is a different matter: [architecture.md](../architecture.md)'s "None added" was argued from the 126 figure and is now **flagged there as needing re-examination**, so do not cite it here as settled.
- Historical rows accumulate — `rejected` and `expired` do not disappear, and now accumulate at twice the old rate — so the **export** is the surface with an unbounded row count, not the queue. See [6-step-03](6-step-03-export-csv.md).
